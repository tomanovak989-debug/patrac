/**
 * MGRS / UTM — NATO NGA algoritmus (proj4js/mgrs, MIT).
...
/** @param {number} lat @param {number} lng @param {number} [accuracy=3] 5=1m … 2=1km @returns {string} */
export function latLngToMgrs(lat, lng, accuracy) {
 return forward([lng, lat], typeof accuracy === 'number' ? accuracy : 3);
}

/** NATO spaced MGRS: 33U UQ 12345 67890 */
export function formatMgrs(mgrsStr) {
 if (!mgrsStr) return '—';
 var s = String(mgrsStr).replace(/\s/g, '').toUpperCase();
 var i = 0;
 while (i < s.length && s.charAt(i) >= '0' && s.charAt(i) <= '9') i++;
 if (i >= s.length - 4) return s;
 var zn = s.substring(0, i);
 var zl = s.charAt(i++);
 var sq = s.substring(i, i + 2);
 i += 2;
 var rest = s.substring(i);
 var half = rest.length / 2;
 return zn + zl + ' ' + sq + ' ' + rest.substring(0, half) + ' ' + rest.substring(half);
}

export function mgrsPrecisionLabel(accuracy) {
 var map = { 5: '1 m', 4: '10 m', 3: '100 m', 2: '1 km', 1: '10 km', 0: '100 km' };
 return map[accuracy] || '';
}

export function latLngToUtm(lat, lng) {
 var utm = LLtoUTM({ lat: lat, lon: lng });
 return {
 easting: utm.easting,
...
export function utmToLatLng(easting, northing, zone, latHint) {
 var zoneLetter = typeof latHint === 'number' ? getLetterDesignator(latHint) : latHint;
 var r = UTMtoLL({ easting: easting, northing: northing, zoneNumber: zone, zoneLetter: zoneLetter });
 return { lat: r.lat, lng: r.lon };
}

export function square100kId(easting, northing, zoneNumber) {
 return get100kID(easting, northing, zoneNumber);
}
